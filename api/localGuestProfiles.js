...
const guestProfilesByName = new Map(
  localGuestProfiles.map((profile) => [normalizeNameKey(profile.name), profile])
);

export const getLocalGuestByEntryId = (rawEntryId) => {
  const entryId = sanitizeEntryId(rawEntryId);
  return entryId ? guestProfilesByEntryId.get(entryId) || null : null;
};

export const getLocalGuestByName = (rawName) => {
