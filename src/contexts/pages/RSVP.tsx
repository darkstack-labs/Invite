...
const RSVP = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem(RSVP_STORAGE_KEY, JSON.stringify(formData));
  ...
  const handleSongSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!songData.name || !songData.entryId || !songData.songName || !songData.artist) {
      toast.error('All fields are required for song request');
      return;
    }
    console.log('Song Request:', songData);
  ...
  const handleSuggestionSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!suggestionData.name || !suggestionData.entryId || !suggestionData.suggestion) {
          ...
          >
            <div className="space-y-2">
              ...
              >
                ...
                <SelectContent className="bg-black/95 border-gold/30">
                  <SelectItem value="veg" className="text-champagne hover:bg-gold/20 focus:bg-gold/20">
                    Vegetarian 🥗
                  </SelectItem>
                  <SelectItem value="nonveg" className="text-champagne hover:bg-gold/20 focus:bg-gold/20">
                    Non-Vegetarian 🍖
                  </SelectItem>
                </SelectContent>
              </Select>
            ...
            <div className="space-y-2">
              <label className="text-gold text-sm font-medium">Dietary Restrictions / Allergies</label>
              <Input
                type="text"
            ...
            {steps.map((step) => (
              ...
              >
                ...
                <span className={`text-xs mt-2 ${
                  currentStep >= step.id ? 'text-gold' : 'text-gold/40'
                }`}>
                  {step.title}
                </span>
              </motion.div>
