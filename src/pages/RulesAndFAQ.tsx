...
const RulesAndFAQ = () => {
  ...
  const [checkedRules, setCheckedRules] = useState<boolean[]>(() => {
    ...
    if (!saved) return new Array(rules.length).fill(false);
    try {
    ...
    return new Array(rules.length).fill(false);
  });
  ...
  const [signed, setSigned] = useState(() => {
    ...
    try {
      const parsed = JSON.parse(saved);
      return Array.isArray(parsed) && parsed.every(Boolean);
    } catch {
      return false;
    }
  });
  ...
  useEffect(() => {
    if (checkedRules.length !== rules.length) {
      const reset = new Array(rules.length).fill(false);
      setCheckedRules(reset);
      localStorage.setItem(RULES_STORAGE_KEY, JSON.stringify(reset));
      return;
    }
  ...
  useEffect(() => {
    if (signed) {
      const shown = sessionStorage.getItem("rules_confetti_shown");

      if (!shown) {
        setShowConfetti(true);
        sessionStorage.setItem("rules_confetti_shown", "true");
      }
    }
  }, [signed]);
                ...
                <ul className="space-y-3">
                  {rules.map((rule, index) => {
                    ...
                    return (
                      <motion.li
                        onClick={() => {
                          if (!signed) handleCheckRule(index);
                        }}
                        key={index}
